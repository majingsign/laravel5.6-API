...
//              }
            ...
            foreach ($users as $key => $val) {
                $user_rest [] = DB::table('temp')->select(['userid','username','riqi','duty_name as type'])
                    ->where(['username'=>$val->user_name])->orderBy('riqi','asc')->get()->toArray();
            }
            //随机获取数据
            $user_temp_user = array();
            foreach ($data_more['date'] as $val) {
                foreach ($users as $key => $user){
                    $user_temp_user [] = DB::table('temp')->select(['userid','username','riqi','duty_name as type'])
                        ->where(['username'=>$user->user_name,'riqi'=>$val])->orderBy('riqi','asc')->inRandomOrder()->first();
                }
            }

            //处理临时数据  E状态只能取3个
            $temp_users   = array();
            foreach ($user_temp_user as $key => $val) {
                foreach ($users as $user) {
                    if ($user->user_id == $val->userid) {
                        static $j = 0;
                        if($val->type == "E"){
                            $j++;
                            if($j == 3){
                                $temp_users [$user->user_id][] = DB::table('temp')->select(['userid','username','riqi','duty_name as type'])
                                    ->where(['username'=>$user->user_name,'riqi'=>$val->riqi])->orderBy('riqi','asc')->whereNotIn('duty_name',['E'])->inRandomOrder()->first();
                                $j = 0;
                            }else {
                                $temp_users [$user->user_id][] = $val;
                            }
                        }else{
                            $temp_users [$user->user_id][] = $val;
                        }
            ...
            foreach ($temp_users as $key => $value ) {
                foreach ($value as $v) {
                    DB::table('temps')->insert(['username'=>$v->username,'riqi'=>$v->riqi,'type'=>$v->type,'userid'=>$v->userid]);
                }
            }
            //查询每个人最后一天上班情况
            $user_last = array();
            foreach ($users as $user) {
                $user_last [] = DB::table('user_scheduling')->select(['last_day','user_id'])->where(['user_id'=>$user->user_id,'duty_time'=>$day_start])->first();
            }
            ...
            //E状态休假天数
            $temp_dates = array();
            foreach ($temp_users_day as $value) {
            ...
            $temp_users_temp = array();
            foreach ($users as $user) {
                $temp_users_temp [] = DB::table('temps')->select(['userid','username','riqi','type'])->where(['username'=>$user->user_name])->get()->toArray();
            }

//            //3、连续五天上班去除2天
            $temp_dates_worker = array();
            foreach ($temp_users_temp as $key => $value) {
...
//                                DB::table('temps')->where(['userid' => $userid])
//                                    ->whereBetween('riqi', [date('Y-m-d', strtotime("$v + 1 day")), date('Y-m-d', strtotime("$v + 2 day"))])
//                                    ->delete();
...
//            exit;
            ...
            $all_last = array();
            foreach ($users as $user) {
                $all_last [] = DB::table('temps')->select(['userid','username','riqi','type'])->where(['username'=>$user->user_name])->get()->toArray();
            }

            //工作天数是否已满
            $all_gt = array();$all_lt = array();$all_eq = array();
            foreach ($all_last as $key => $value) {
                //判断是否存在E情况
...
//                      $all_lt [$key] = $all_lt_temp;
            ...
            $all_lasts = array_merge($all_gt,$all_lt,$all_eq);
            $everyone  = array();
            $time_temp = array(); //工作天数
             foreach ($all_lasts as $value){
                 if(is_array($value) || is_object($value)){
                     $userid    = array_unique(array_column($value,"userid"))[0];
                     $data = array();
...
//              //计算每人每天工作日要上班
//              $all_gt = array();$all_lt = array();$all_eq = array();
//              foreach ($user_rest as $key => $value){
//                  $num  = count($value);
//                  if($num > $day_worker){
//                      $keys  = array_rand((array)$value,$day_worker);
//                      foreach ($keys as $k => $ks){
...
//                          ->whereIn('userid',array_unique(array_column($value,'userid')));
//                            if(count($temp_dates) != $userscount){
//                                echo "<javascript>alert('排班生成失败,请重新生成')</javascript>";
//                            }else {
//                                $query->whereNotIn('riqi', array_merge(array_column($value, 'riqi'), $temp_dates[array_unique(array_column($value, 'userid'))[0]]));
//                            }
//                          $all_lt_add = $query->groupBy(['riqi'])
...
////                                    echo "<javascript>alert('排班生成失败,请重新生成')</javascript>";
////                                } else {
////                                    $query->whereNotIn('riqi', array_unique(array_merge($temp_dates_worker[array_unique(array_column($value, 'userid'))[0]],$temp_dates[array_unique(array_column($value, 'userid'))[0]])));
////                                }
////                                $all_lt_add = $query->groupBy(['riqi'])
////                                    ->limit($day_worker)
////                                    ->inRandomOrder()->get()
